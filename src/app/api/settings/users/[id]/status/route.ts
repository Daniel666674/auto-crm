...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Privilege-escalation guard: marketing may not activate/deactivate a superadmin.
  if (role === "marketing" && target.role.replace(/^inactive:/, "") === "superadmin") {
    return NextResponse.json({ error: "Marketing no puede modificar superadmin" }, { status: 403 });
  }
