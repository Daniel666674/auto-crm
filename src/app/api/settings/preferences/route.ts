...
import { BRAND_PRESET_ORDER, SALES_DEFAULT_PRESET, getBrandPreset } from "@/lib/brand-presets";

const defaultPreset = getBrandPreset(SALES_DEFAULT_PRESET);
...
export async function GET() {
  ...
  try {
    ...
    if (!ALLOWED_PRESETS.has(merged.theme)) merged.theme = SALES_DEFAULT_PRESET;
...
export async function PUT(req: NextRequest) {
  ...
  if (typeof merged.theme === "string" && !ALLOWED_PRESETS.has(merged.theme as string)) {
    merged.theme = SALES_DEFAULT_PRESET;
  }
